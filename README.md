# Tutorial Passo a Passo para a Construção da Aplicação TechLoja 2.0

## Passo 1: Preparação do Ambiente de Desenvolvimento

1. Instale o **Node.js** (versão 18 ou superior) em [nodejs.org](https://nodejs.org).
2. Instale um servidor MySQL (recomendo **MySQL Community Server** ou **XAMPP** para facilitar o phpMyAdmin).
3. Instale o **Visual Studio Code** (com extensões: Live Server, ESLint, Prettier — opcional, mas útil).
4. Crie uma pasta principal chamada `techloja` no seu computador.
5. Dentro dela, crie estas subpastas:
    - `backend`
    - `frontend`
    - `sql`
    - `public` (para arquivos estáticos do frontend)

**Dica:** Organize seu ambiente antes de começar. Isso evita frustrações depois!

---

## Passo 2: Configurar o Banco de Dados (MySQL)

1. Crie o arquivo `sql/criar_banco_loja_tech.sql` e cole o seguinte código completo:
